...
const data = {
  ...
  colors: ['#685A34', '#A7A87D', '#4C8051', '#7D3B53', '#50487C', '#BEEEFF'],
  legend: {
    enabled: false
  },
  series: [{}]
...
export interface OperationsWasteProductionByCategoryCardProps {
  style?: CSSProperties;
  list: any;
  condensed?: boolean;
  tabletMode?: boolean;
...
export const OperationsWasteProductionByCategoryCard = ({
  style,
  list,
  condensed = false,
  tabletMode = false,
  mobileMode = false
}: OperationsWasteProductionByCategoryCardProps) => {
  ...
  list?.chart?.forEach((element: any) => {
    element.data.forEach((el: any) => {
      if (el.production !== 0 || el.y !== 0) hasData = true;
    });
  });

  data.xAxis.categories = ['Organic', 'Non Recyclable', 'Recyclable', 'Hazardous'];
  if (condensed) {
  ...
  return (
    <div style={{...styles.container, ...style, ...(condensed ? styles.condensedContainer : {})}}>
      <div style={{...styles.titleWrapper, ...(condensed ? styles.condensedTitleWrapper : {})}}>
        <Text variant="cardHeader">{'OPERATIONS WASTE DATA BY CATEGORY'}</Text>
      </div>
      <div style={{...styles.sectionsWrapper, ...(!hasData ? {flexDirection: 'column'} : {})}}>
        {hasData ? (
